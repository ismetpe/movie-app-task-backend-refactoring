...
namespace UnitTests
{
    ...
    class UserTicketsTest
    {
        ...
        ScreeningsService _screeningsService;
        ...
        public async Task Setup()
        {
            ...
            _context.Ratings.Add(new Rating { Id = 9, Rating_value = 3.0F, MediaId = 3 });
             _context.Screenings.Add(new Screening { Id = 45, Date = System.DateTime.Now.AddDays(100), MediaId = 83, Number_of_seats = 100, Number_of_tickets = 100, Place = "Sarajevo", Time = "11:00" });
            _context.Users.Add(new User { Id = 1, Username = "user1", Password = "123456" });


            await _context.SaveChangesAsync();
            _mediaService = new MediaService(_mapper, _context);
            _screeningsService = new ScreeningsService(_context, _mapper);
        }
        ...
        public async Task Buying_Ticket_After_Screening()
        {
            int result = await _screeningsService.BuyTickets(1, 45, 2, DateTime.Now.AddDays(101));
        ...
        public async Task Buying_Ticket_Before_Screening()
        {


            Assert.Catch<Exception>(() => _screeningsService.BuyTickets(1, 45, 2, DateTime.Now.AddDays(88)), null);  
        }


        [Test]
        public async Task Buyin_10_Tickets()
        {
            int result = await _screeningsService.BuyTickets(1, 45, 10, DateTime.Now.AddDays(101));
